...
const getOutofOrder = async (startDate) => {
  const stayFrom = moment(startDate).subtract(1, 'month').format('YYYY-MM-DD');
  const stayTo = moment(stayFrom).add(2, 'month').format('YYYY-MM-DD');
  try {
...
export default async (req, res) => {
  try {
    ...
    const monthFromToday = moment().add(8, 'month').format('YYYY-MM-DD');
    ...
    const month = result.availability.inventory.inventoryItem.map((e) => {
      const availabilityToOccupancy = (baseCode, availability) => {
        let occupancy = 0;
        if (baseCode.includes('SUP')) occupancy = 1 - availability;
